  ...
  useEffect(() => {
    const fetchInformacoesVaga = async () => {
      try {
        ...
        const candidatosComDocumento = handleAnexaDocumentos(candidatosResponse.data, response.data.candidatos)
        const filtrosResponse = await fetchFiltros()
        console.log(candidatosComDocumento)
        setInformacoesVaga(response.data)
        setCandidato(candidatosComDocumento)
    ...
    const handleAnexaDocumentos = (candidatos, documentos) => {
      return candidatos.map(candidato => {
        const documentoEncontrado = documentos.find(doc => doc.idCandidato === candidato.id);

        if (documentoEncontrado) {
          return {
            ...candidato,
            curriculo: documentoEncontrado.curriculo,
            cartaApresentacao: documentoEncontrado.cartaApresentacao,
            portifolio: documentoEncontrado.portifolio
          };
        }
